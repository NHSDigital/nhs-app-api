...
namespace NHSOnline.Backend.GpSystems.Suppliers.Emis.PatientRecord
{
    public class GetMedicationsTaskChecker
    {
        ...
        public Medications Check(Task<EmisApiObjectResponse<MedicationRootObject>> task)
        {
            _logger.LogEnter();
            ...
            if (!medicationsResponse.HasSuccessResponse)
            {
                ...
                {
                    _logger.LogWarning("User does not have access to their medications in their patient record");
                    medications = new Medications
                    {
                        HasAccess = false
                    };
