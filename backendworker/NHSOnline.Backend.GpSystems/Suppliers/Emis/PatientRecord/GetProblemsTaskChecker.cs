...
{
    internal sealed class GetProblemsTaskChecker
    {
        ...
        public Problems Check(Task<EmisApiObjectResponse<MedicationRootObject>> task)
        {
            _logger.LogEnter();
            ...
            if (!problemsResponse.HasSuccessResponse)
            {
                ...
                {
                    _logger.LogWarning("User does not have access to their problems in their patient record");
                    problems = new Problems
                    {
                        HasAccess = false
                    };
