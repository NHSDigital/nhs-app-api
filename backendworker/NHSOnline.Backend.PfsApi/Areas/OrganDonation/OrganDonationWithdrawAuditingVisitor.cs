...
namespace NHSOnline.Backend.PfsApi.Areas.OrganDonation
{
    public class OrganDonationWithdrawAuditingVisitor : IOrganDonationWithdrawResultVisitor<Task>
    {
        ...
        {
            await _auditor.PostOperationAudit(AuditType, "The organ donation decision has been successfully Withdrawn");
        ...
        {
            await _auditor.PostOperationAudit(AuditType, "There was an issue withdrawing the organ donation decision");
        }

        public async Task Visit(OrganDonationWithdrawResult.UpstreamError result)
        {
            await _auditor.PostOperationAudit(AuditType, "There was an upstream error when withdrawing the organ donation decision");
        }

        public async Task Visit(OrganDonationWithdrawResult.Timeout result)
        {
            await _auditor.PostOperationAudit(AuditType, "The organ donation Withdraw system took too long to respond");
        }

        public async Task Visit(OrganDonationWithdrawResult.BadRequest result)
        {
            await _auditor.PostOperationAudit(AuditType, "The organ donation withdraw request failed validation");
        }
    }
}
