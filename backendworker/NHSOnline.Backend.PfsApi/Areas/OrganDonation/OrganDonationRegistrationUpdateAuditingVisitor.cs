...
namespace NHSOnline.Backend.PfsApi.Areas.OrganDonation
{
    public class OrganDonationRegistrationUpdateAuditingVisitor : IOrganDonationRegistrationResultVisitor<Task>
    {
        ...
        public OrganDonationRegistrationUpdateAuditingVisitor(IAuditor auditor, IMetricLogger<UserSessionMetricContext> metricLogger,
        ...
        {
            await _auditor.PostOperationAudit(AuditType, "The organ donation decision has been successfully updated");
        ...
        {
            await _auditor.PostOperationAudit(AuditType, "There was an issue registering the organ donation decision update");
        }

        public async Task Visit(OrganDonationRegistrationResult.UpstreamError result)
        {
            await _auditor.PostOperationAudit(AuditType, "There was an upstream error when registering the organ donation decision update");
        }

        public async Task Visit(OrganDonationRegistrationResult.Timeout result)
        {
            await _auditor.PostOperationAudit(AuditType, "The organ donation registration update system took too long to respond");
        }

        public async Task Visit(OrganDonationRegistrationResult.BadRequest result)
        {
            await _auditor.PostOperationAudit(AuditType, "The organ donation update registration request failed validation");
        }
    }
}
