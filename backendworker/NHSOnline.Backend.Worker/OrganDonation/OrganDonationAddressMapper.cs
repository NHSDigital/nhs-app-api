...
{
    internal class OrganDonationAddressMapper : IMapper<string, Areas.OrganDonation.Models.Address, Address>
    {
        private static readonly Regex PostCodeRegex =
            new Regex(@"([A-Za-z][A-Ha-hJ-Yj-y]?[0-9][A-Za-z0-9]? ?[0-9][A-Za-z]{2}|[Gg][Ii][Rr] ?0[Aa]{2})");

        public Address Map(string firstSource, Areas.OrganDonation.Models.Address secondSource)
        {
        ...
        private Address Map(Areas.OrganDonation.Models.Address address) => address != null
            ? new Address
            {
