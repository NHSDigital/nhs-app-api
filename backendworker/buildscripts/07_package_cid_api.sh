...
then
  ...
  push_docker_image "${DOCKER_REGISTRY}/nhsonline-backendcidapi:${DOCKER_TAG}"
  push_docker_image "${DOCKER_REGISTRY}/nhsonline-backendcidapi:${BRANCH_TAG}"
  if [ -n "${DOCKER_REGISTRY_RELEASE}" ]
  then
    docker tag "${DOCKER_REGISTRY}/nhsonline-backendcidapi:${DOCKER_TAG}" "${DOCKER_REGISTRY_RELEASE}/nhsonline-backendcidapi:${BRANCH_TAG}"
    push_docker_image "${DOCKER_REGISTRY_RELEASE}/nhsonline-backendcidapi:${BRANCH_TAG}"
  fi
fi