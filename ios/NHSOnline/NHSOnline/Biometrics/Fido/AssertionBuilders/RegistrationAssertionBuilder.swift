...
class RegistrationAssertionBuilder: FidoAssertionBuilder{
    ...
    @available(iOS 10.0, *)
    override func getAssertions(response: FidoResponse) throws -> String {
        var value = [UInt8]()
        var buffer = [UInt8]()
        do {
            try value = getRegistrationAssertion(response)
    ...
    @available(iOS 10.0, *)
    func getRegistrationAssertion(_ response: FidoResponse) throws -> [UInt8] {
        do {
            var buffer = [UInt8]()
            var value: [UInt8] = try getSignedData(response)
    ...
    @available(iOS 10.0, *)
    func getSignedData(_ response: FidoResponse) throws -> [UInt8] {
        do {
            var buffer = [UInt8]()
            
            
            buffer.append(contentsOf: EncodingHelper.encodeInt(TagsEnum.TAG_AAID.rawValue))
            var value: [UInt8] =  Array(config().AAID.utf8)
            buffer.append(contentsOf: EncodingHelper.encodeInt(value.count))
            buffer.append(contentsOf: value)
