...
class AuthenticationAssertionBuilder: FidoAssertionBuilder {
    ...
    override func getAssertions(response: FidoResponse) throws -> String {
        do {
            ...
            try value = getAuthAssertion(response)
    ...
    func getDataToSign(_ response: FidoResponse) throws -> [UInt8] {
    ...
    @available(iOS 10.0, *)
    func getAuthAssertion(_ response: FidoResponse) throws -> [UInt8] {
        do {
            var buffer = [UInt8]()
            var length: Int = 0
            var value: [UInt8] = []
            
            value = try getDataToSign(response)
            length = value.count
