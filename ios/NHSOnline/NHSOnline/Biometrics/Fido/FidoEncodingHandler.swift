...
class FidoEncodingHandler {
    ...
    @available(iOS 10.0, *)
    func getEncodedRegistrationResponse(with registrationRequest: RegistrationRequest) throws -> String {
        let response: FidoResponse
        do {
            try response = requestHandler.getRegistrationResponse(with: registrationRequest)
