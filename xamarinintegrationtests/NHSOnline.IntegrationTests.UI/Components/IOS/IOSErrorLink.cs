...
{
    public sealed class IOSErrorLink
    {
        private readonly IIOSInteractor _interactor;
        private readonly string _text;

        private IOSErrorLink(IIOSInteractor interactor, string text)
        {
            _interactor = interactor;
            _text = text;
        }

        public static IOSErrorLink WithText(IIOSInteractor interactor, string text)
            => new IOSErrorLink(interactor, text);
        ...
        private By FindBy
            => MobileBy.IosNSPredicate($"type == 'XCUIElementTypeLink' AND name == {_text.QuotePredicateLiteral()}");
    }
}