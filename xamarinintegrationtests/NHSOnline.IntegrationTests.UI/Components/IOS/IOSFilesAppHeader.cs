...
{
    public sealed class IOSFilesAppHeader
    {
        private readonly IIOSInteractor _interactor;
        private readonly IIOSLocatorStrategy _locatorStrategy;

        private IOSFilesAppHeader(IIOSInteractor interactor, IIOSLocatorStrategy locatorStrategy)
        {
            _interactor = interactor;
            _locatorStrategy = locatorStrategy;
        }

        public static IOSFilesAppHeader WithText(IIOSInteractor interactor, string text)
            => new IOSFilesAppHeader(interactor, new TextLocatorStrategy(interactor, text));

        public void AssertVisible() => _locatorStrategy.ActOnElementContext(
            context => context.Element.Displayed.Should().BeTrue($"a document header {_locatorStrategy.Description} should be displayed"));
        ...
        private sealed class TextLocatorStrategy : IIOSLocatorStrategy
        {
            ...
            public By FindBy => MobileBy.IosNSPredicate($"type == 'XCUIElementTypeOther' AND label CONTAINS {_text.QuotePredicateLiteral()}");
