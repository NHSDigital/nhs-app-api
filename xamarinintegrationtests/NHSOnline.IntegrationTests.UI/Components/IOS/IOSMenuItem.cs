...
namespace NHSOnline.IntegrationTests.UI.Components.IOS
{
    public class IOSMenuItem
    {
        ...
        private IOSMenuItem(IIOSInteractor interactor, IIOSLocatorStrategy locatorStrategy)
        {
            _interactor = interactor;
            _locatorStrategy = locatorStrategy;
        }

        public static IOSMenuItem StartsWith(IIOSInteractor interactor, string text)
            => new IOSMenuItem(interactor, new TextLocatorStrategy(interactor, text));
        ...
        private sealed class TextLocatorStrategy : IIOSLocatorStrategy
        {
            ...
            public By FindBy =>
                By.XPath($"//XCUIElementTypeLink[starts-with(@label,{_text.QuoteXPathLiteral()})]");
