...
Currently this is limited to generating the default configuration. If you want to run web locally with non-default configuration, see the [running npm run dev with non-default vars](../how-tos/non-default-npm-run-dev.md) documentation for a work-around that works with any `make` command.