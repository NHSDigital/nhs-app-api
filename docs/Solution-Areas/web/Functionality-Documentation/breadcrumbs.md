...
All breadcrumbs are configured in **src / breadcrumbs** directory. Configurations have been grouped based on the area/feature of the app in which they exist, and pretty much map directly to the associated routes configuration files.
...
- When referenced in another breadcrumb configuration's `defaultCrumb` (or alternative crumb) properties
  - **name**: the name (**src / router / names.js**) of the route to navigate to when this crumb is clicked in the breadcrumb trail
  - **i18nKey**: the locale key to use for the crumb label

- When used as a `crumb` value on a route's meta configuration
...
_Note: If a route has no breadcrumb either on desktop or native, set its configuration `crumb` to be `{}`_
...
See [Router](router.md) docs for more information on route configurations