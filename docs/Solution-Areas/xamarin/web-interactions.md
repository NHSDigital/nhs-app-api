...
Any parameters to these function calls need to be converted from JSON to C# objects or vice versa. This conversion is always done in the appropriate WebView using classes defined in the `Payloads` namespace. These classes should just be used for passing the data from the WebView to the presenter or from the presenter to the WebView. Any services used by the presenter should have their own interfaces decoupled from the javascript payloads. This reduces the risk of inadvertently changing something that breaks the javascript interaction.
