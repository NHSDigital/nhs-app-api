...
Feature: Patients Know Best Secondary Care Medicines
  ...
  Scenario: A user navigates directly to an external partner site and will see a warning page
    Given I am a user who can view Medicines from PKB Secondary Care
    And Secondary Care responds to requests for medicines
    And I am logged in
    When I navigate to the redirector page with a url of '/redirector?redirect_to=http%3A%2F%2Fpkb.stubs.local.bitraft.io%3A8080%2Fnhs-login%2Flogin%3FphrPath%3D%252Fauth%252FmanageMedications.action%253Ftab%253Dtreatments%26brand%3DpkbSecondaryCare'
    Then I am redirected to the redirector page with the header 'Hospital and other medicines'
    When I click the 'Continue' button on the redirector page with a url starting with 'http://pkb.stubs.local.bitraft.io:8080/nhs-login/login?phrPath=%2Fauth%2FmanageMedications.action%3Ftab%3Dtreatments&brand=pkbSecondaryCare'
