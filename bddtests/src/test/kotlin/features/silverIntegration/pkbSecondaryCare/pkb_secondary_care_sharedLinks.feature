...
@pkbSecondaryCare
@sharedlinks
Feature: Patients Know Best Secondary Care Shared Links
  ...
  Scenario: A user navigates to PKB Secondary Care shared links and sees the warning message
    Given I am using the native app user agent
    And I am a user who can view Shared Links from PKB Secondary Care
    And I am logged in
    ...
    And the shared health links warning on the page explains the service is from PKB Secondary Care
  ...
  Scenario: A user without access to PKB My Care View cannot see the menu item 'Shared links' on the health record hub page
    ...
    When I navigate to the Health Record Hub page
    Then I see the health records hub page
    And the link to PKB Secondary Care shared links is not available on the Health Records Hub
  ...
  Scenario: A user can follow the link to Find out more about personal health records
    ...
    And I am logged in
    When I navigate to the redirector page with a url of '/redirector?redirect_to=http%3A%2F%2Fpkb.stubs.local.bitraft.io%3A8080%2Fnhs-login%2Flogin%3FphrPath%3D%252Flibrary%252FmanageLibrary.action%26brand%3DpkbSecondaryCare'
  ...
  Scenario: A user navigates to an external partner site and will see a warning page
    Given I am a user who can view Shared Links from PKB Secondary Care
    And Secondary Care responds to requests for shared links
    And I am logged in
    When I navigate to the redirector page with a url of '/redirector?redirect_to=http%3A%2F%2Fpkb.stubs.local.bitraft.io%3A8080%2Fnhs-login%2Flogin%3FphrPath%3D%252Flibrary%252FmanageLibrary.action%26brand%3DpkbSecondaryCare'
    Then I am redirected to the redirector page with the header 'Shared health links'
    When I click the 'Continue' button on the redirector page with a url starting with 'http://pkb.stubs.local.bitraft.io:8080/nhs-login/login?phrPath=%2Flibrary%2FmanageLibrary.action&brand=pkbSecondaryCare'
