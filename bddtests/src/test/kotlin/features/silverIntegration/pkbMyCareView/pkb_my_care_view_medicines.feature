@silverIntegration
@pkbMyCareView
Feature: Patients Know Best My Care View Medicines
  ...
  Scenario: A user navigates to PKB My Care View Medicines and sees the warning message
    Given I am using the native app user agent
    And I am a user who can view Medicines from PKB My Care View
    And I am logged in
    And I navigate to the your prescriptions page
    Then the Prescriptions Hub page is displayed
    And the PKB My Care View Medicines link is available on the Prescriptions Hub
    When I click the PKB My Care View Medicines link on the Prescriptions hub
  ...
  Scenario: A user navigates directly to an external partner site and will see a warning page
    ...
    And I am logged in
    When I navigate to the redirector page with a url of '/redirector?redirect_to=http%3A%2F%2Fpkb.stubs.local.bitraft.io%3A8080%2Fnhs-login%2Flogin%3FphrPath%3D%252Fauth%252FmanageMedications.action%253Ftab%253Dtreatments%26brand%3DpkbMyCareView'
    Then I am redirected to the redirector page with the header 'Hospital and other medicines'
    When I click the 'Continue' button on the redirector page with a url starting with 'http://pkb.stubs.local.bitraft.io:8080/nhs-login/login?phrPath=%2Fauth%2FmanageMedications.action%3Ftab%3Dtreatments&brand=pkbMyCareView'
