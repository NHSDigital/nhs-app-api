...
@patientsKnowBest
@sharedlinks
Feature: Patients Know Best Record Sharing
  ...
  Scenario: A user navigates to PKB Record Sharing and sees the warning message
    Given I am using the native app user agent
    And I am a user who can view Record Sharing from Patients Know Best
    And I am logged in
    ...
    And the record sharing warning on the page explains the service is from Patients Know Best

  Scenario: A user without access to PKB cannot see the menu item 'Record Sharing' on the health record hub page
  ...
  Scenario: A user navigates to an external partner site and will see a warning page
    ...
    When I click the 'Continue' button on the redirector page with a url starting with 'http://pkb.stubs.local.bitraft.io:8080/nhs-login/login?phrPath=/patient/myConsentTeam.action?tab=invitations&subTab=myClinicians'
