...
Feature: Care Information Exchange Test Results
  ...
  Scenario: A user navigates to CIE test results and sees the warning message
    Given I am using the native app user agent
    And I am a user who can view test results from Care Information Exchange
    And I am logged in
    ...
    And the test results warning on the page explains the service is from Care Information Exchange

  Scenario: A user without access to CIE cannot see the menu item 'Test results' on the Health Record Hub page
    Given I am a user who cannot view test results from Care Information Exchange
    And I am logged in
    When I navigate to the health record hub page
    Then I see the health records hub page
    And the link to CIE test results is not available on the health record hub page
  ...
  Scenario: A user can follow the link to Find out more about personal health records
    ...
    And I am logged in
    When I navigate to the redirector page with a url of '/redirector?redirect_to=http%3A%2F%2Fpkb.stubs.local.bitraft.io%3A8080%2Fnhs-login%2Flogin%3FphrPath%3D%252Ftest%252FmyTests.action%26brand%3Dcie'
  ...
  Scenario: A user navigates directly to an external partner site and will see a warning page
    Given I am a user who can view test results from Care Information Exchange
    And CIE responds to requests for test results
    And I am logged in
    When I navigate to the redirector page with a url of '/redirector?redirect_to=http%3A%2F%2Fpkb.stubs.local.bitraft.io%3A8080%2Fnhs-login%2Flogin%3FphrPath%3D%252Ftest%252FmyTests.action%26brand%3Dcie'
    Then I am redirected to the redirector page with the header 'Test results'
    When I click the 'Continue' button on the redirector page with a url starting with 'http://pkb.stubs.local.bitraft.io:8080/nhs-login/login?phrPath=%2Ftest%2FmyTests.action&brand=cie'
