...
Feature: Care Information Exchange Shared Links
  ...
  Scenario: A user can follow the link to Find out more about personal health records
    ...
    And I am logged in
    When I navigate to the redirector page with a url of '/redirector?redirect_to=http%3A%2F%2Fpkb.stubs.local.bitraft.io%3A8080%2Fnhs-login%2Flogin%3FphrPath%3D%252Flibrary%252FmanageLibrary.action%26brand%3Dcie'
  ...
  Scenario: A user navigates directly to an external partner site and will see a warning page
    ...
    When I navigate to the redirector page with a url of '/redirector?redirect_to=http%3A%2F%2Fpkb.stubs.local.bitraft.io%3A8080%2Fnhs-login%2Flogin%3FphrPath%3D%252Flibrary%252FmanageLibrary.action%26brand%3Dcie'
    Then I am redirected to the redirector page with the header 'Shared health links'
    When I click the 'Continue' button on the redirector page with a url starting with 'http://pkb.stubs.local.bitraft.io:8080/nhs-login/login?phrPath=%2Flibrary%2FmanageLibrary.action&brand=cie'
