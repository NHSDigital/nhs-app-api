...
Feature: Organ Donation Amend Backend
  ...
  Scenario Outline: When amending an organ donation decision to opt in, a registered <GP System> user receives a 200 response and a registration id
  ...
  Scenario Outline: When amending an organ donation decision, an OD response of <Error Code> will prompt a 502 response and a retry option
    Given I am a EMIS api user amending my decision, but OD returns '<Error Code>' error
    And I have logged in and have a valid session cookie
    When I submit my updated decision to organ donation
  ...
  Scenario Outline: When amending an organ donation decision, an OD response of <Error Code> will prompt a 502 response and no retry option
    Given I am a EMIS api user amending my decision, but OD returns '<Error Code>' error
    And I have logged in and have a valid session cookie
    When I submit my updated decision to organ donation
    Then I receive a "Bad Gateway" error
