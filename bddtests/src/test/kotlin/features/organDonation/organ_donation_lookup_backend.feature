...
Feature: Organ Donation Lookup Backend
  ...
  Scenario Outline: When looking for an organ donation registration, an OD response of <Error Code> will prompt a 500 response and no retry option
  ...
  Scenario Outline: When looking for an organ donation registration, an OD response of <Error Code> will prompt a 502 response and a retry option
    Given I am a EMIS api user registered with OD, but lookup returns '<Error Code>' error
    And I have logged in and have a valid session cookie
    When I request my organ donation details
  ...
  Scenario Outline: When looking for an organ donation registration, an OD response of <Error Code> will prompt a 502 response and no retry option
    Given I am a EMIS api user registered with OD, but lookup returns '<Error Code>' error
    And I have logged in and have a valid session cookie
    When I request my organ donation details
    Then I receive a "Bad Gateway" error
  ...
  Scenario Outline: When looking for an organ donation registration but <GP System> fails to return demographics, the user receives a 502 response
