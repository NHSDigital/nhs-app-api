...
open class AvailableAppointmentFilterSteps : AppointmentsBookingData() {
    ...
    fun selectFilterOptionsToRevealSlots() {
        ...
        if (!filterValues.type.isNullOrEmpty()) availableAppointmentsPage.appointmentTypeFilter.selectByText(filterValues.type!!)
        if (!filterValues.location.isNullOrEmpty()) availableAppointmentsPage.locationFilter.selectByText(filterValues.location!!)
        if (!filterValues.doctor.isNullOrEmpty()) availableAppointmentsPage.clinicianFilter.selectByText(filterValues.doctor!!)
    }
