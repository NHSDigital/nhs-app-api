...
Feature: Error scenarios for nominated pharmacy
  ...
  Scenario Outline: Patient does not see nominated pharmacy when call to NHSSearch to retrieve EPS status fails
    Given I am a patient using the <GP System> GP System
    And I have 1 past repeat prescriptions
    And each repeat prescription contains 1 courses of which 1 are repeats
    And the request to Azure search to retrieve my practice details fails
    When I am logged in
    And I navigate to prescriptions
    Then the Prescriptions Hub page is displayed
    And I do not see the nominated pharmacy panel
