...
class TppRequestBinaryDataBuilder(tppUserSession: TppUserSession) : TppMappingBuilder("POST", "/tpp/") {
    init {
        val typeHeader = "type"
        val typeValue = "RequestBinaryData"
        val apiVersion = "1"

        requestBuilder
                .andHeader(typeHeader, typeValue)
                .andBodyMatchingXpath("//RequestBinaryData[" +
                        "@apiVersion='${apiVersion}' and " +
                        "@patientId='${tppUserSession.patientId}' and " +
                        "@onlineUserId='${tppUserSession.onlineUserId}' and " +
                        "@binaryDataId='123456433546' and " +
    ...
    fun respondWithSuccess(requestBinaryDataReply: RequestBinaryDataReply): Mapping {
        ...
        stringWriter.use {
            marshaller.marshal(requestBinaryDataReply, stringWriter)
        }
