...
const mountComponent = ({
  ...
  wrapper = mount(MessageDialogGeneric, { propsData, $store, methods });
...
describe('MessageDialogGeneric', () => {
  ...
  describe('lifecycle hooks', () => {
    it('will register FOCUS_ERROR_ELEMENT event in beforeMount', () => {
      MessageDialogGeneric.beforeMount();
      expect(EventBus.$on).toHaveBeenCalledWith(
        FOCUS_ERROR_ELEMENT, MessageDialogGeneric.scrollToTopAndFocusDialog,
      );
    });

    it('will deregister FOCUS_ERROR_ELEMENT event in beforeDestroy', () => {
      MessageDialogGeneric.beforeDestroy();
