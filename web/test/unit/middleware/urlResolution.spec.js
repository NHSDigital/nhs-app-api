  ...
  describe('server', () => {
    ...
    describe('x-origin header exists', () => {
      ...
      describe('x-origin is prefixed wiht www- ', () => {
        ...
        it('will correctly set the CID_REDIRECT_URI from the `x-origin` header', () => {
          expect(input.env.CID_REDIRECT_URI).toBe('http://web-purple.some.web.host/auth-return');
        });

        it('will correctly set the CID_REDIRECT_URI_NATIVE from the `x-origin` header', () => {
          expect(input.env.NATIVE_CID_REDIRECT_URI).toBe('nhsapp://web-purple.some.web.host/auth-return');
        });
      });
    ...
    describe('x-origin header does not exist', () => {
      beforeEach(() => {
        input.req.headers.host = 'www.foo.bar';
