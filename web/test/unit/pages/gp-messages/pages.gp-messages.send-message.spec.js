...
describe('patient messaging messages', () => {
  ...
  describe('methods', () => {
    it('will send the message if the inputs are valid', async () => {
      ...
      expect(store.dispatch).toHaveBeenCalledWith('gpMessages/setSelectedMessageID', '0');
      expect(store.dispatch).toHaveBeenCalledWith('gpMessages/sendMessage', {
    ...
    it('will not send the message if the inputs are invalid', async () => {
      ...
      expect(store.dispatch).not.toHaveBeenCalledWith('gpMessages/setSelectedMessageID', '0');
      expect(store.dispatch).not.toHaveBeenCalledWith('gpMessages/sendMessage', {
        messageText: 'Test message',
        subjectText: 'Test subject',
      });
