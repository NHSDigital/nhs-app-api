        ...
        <Card data-label="allergies-and-reactions">
          <div data-purpose="test-results-card">
            <span v-if="testResult.date.value">
              <strong>{{ testResult.date.value | datePart(testResult.date.datePart) }}</strong>
            </span>
            <span v-else>
            ...
            <ul
              v-if="testResult.associatedTexts.length !== 0"
              class="nhsuk-list nhsuk-list--bullet"
            >
              <li
                v-for="(associatedText, associatedTextItemIndex) in testResult.associatedTexts"
                :key="`associatedText-${associatedTextItemIndex}`"
              >{{ associatedText }}</li>
            </ul>
            <ul
              v-if="testResult.testResultChildLineItems.length !== 0"
              class="nhsuk-list nhsuk-list--bullet"
            >
              <li
                v-for="(lineItem, lineItemIndex) in testResult.testResultChildLineItems"
                :key="`line-${lineItemIndex}`"
              >
                {{ lineItem.description }}
                <ul
                  v-if="lineItem.associatedTexts.length !== 0"
                  class="nhsuk-list nhsuk-list--bullet"
                >
                  <li
                    v-for="(lineItemAssociatedText, lineItemAssociatedTextIndex)
                      in lineItem.associatedTexts"
                    :key="`lineAssociatedText-${lineItemAssociatedTextIndex}`"
                  >{{ lineItemAssociatedText }}</li>
                </ul>
              </li>
            </ul>
          </div>
