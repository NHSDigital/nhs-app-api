<template>
  <div>
    ...
    <div v-else>
      <p>{{ $t('organDonation.viewDecision.nextSteps.optOutText') }}</p>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      shareDecisionUrl: this.$store.app.$env.ORGAN_DONATION_SHARE_DECISION_URL,
      tellFamilyUrl: this.$store.app.$env.ORGAN_DONATION_TELL_FAMILY_URL,
    };
  },
};
</script>
